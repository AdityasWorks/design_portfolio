...
const ProjectCard = ({ project, className = "", videoLoop = false }) => {
    ...
    // Initialize video playback when component mounts and is visible
    useEffect(() => {
        if (!isVisible || !videoRef.current || project.type !== "video") return;
    ...
    }, [isVisible, project.type, project.videoUrl, videoLoop]);
    ...
    return (
        ...
        >
            ...
            <div className="absolute inset-0  p-4 sm:p-5 flex flex-col justify-end z-[2]">
                {project.category && (
                    <span className="text-white/100 text-xs uppercase tracking-wider mb-1">{project.category}</span>
                )}

                {project.title && (
                    <h3 className="text-white/100 text-base sm:text-lg font-bold mb-1">{project.title}</h3>
                )}

                {project.description && <p className="text-white/90 text-xs sm:text-sm">{project.description}</p>}
            </div>
        </motion.div>
    );
};
